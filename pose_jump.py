...
#保存關鍵點數據和角度：在 CSV 文件中保存每個關鍵點的 (x, y, z) 坐標，以及計算的角度和動作標籤（"jump"或"other"）
#跳躍判斷：利用腳踝、膝蓋、髖部的角度變化來判斷腿部在準備跳躍的狀態，及檢查人體重心的垂直位移（例如腳部的高度變化），來判斷跳躍的進行
#角度範圍擴展：跳躍時膝蓋角度範圍60到130度擴展10%，變為54到143度，讓判斷更加包容動作不標準的情況。另腳踝高度變化avg_ankle_height < -0.3
#輸出檔案：只有當 action_label == "jump" 時，才將數據寫入 CSV 檔案
...
# 設定影片和輸出圖片、CSV 路徑
video_path = 'D:/Pose Train/jump1.mp4'
...
with mp_pose.Pose(
    ...
    with open(output_csv_path, mode='w', newline='') as file:
        ...
        # CSV 標頭：每個關鍵點的 x, y, z 坐標，並加上膝蓋角度和動作標籤
        ...
        while cap.isOpened() and image_count < num_images:
            ...
            if results.pose_landmarks and frame_count % interval == 0:
                ...
                avg_ankle_height = (left_ankle_z + right_ankle_z) / 2

                # 判斷角度和高度變化以確定是否為跳躍動作（擴展10%範圍）
                if 54 < knee_angle < 143 and avg_ankle_height < -0.3:
                    action_label = "jump"
                else:
                    action_label = "other"

                # 只有當動作標記為 "jump" 才將數據保存到 CSV
                if action_label == "jump":
